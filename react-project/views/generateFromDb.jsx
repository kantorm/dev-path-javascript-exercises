                        ...
                        return <form key={survey._id} action={action} method='POST' name={survey.surveyName}>
                            ...
                            {survey.questions.map(function(question) {
                                return <div key={survey._id + question.questionName}>
                                    ...
                                    {question.answers.map(function(answerOption) {
                                        if (question.fieldType != 'text')
                                            return <label key={survey._id + Math.random()}><input name={question.questionName} type={question.fieldType} value={answerOption}/>{answerOption}</label>
                                        else
                                            return <label key={survey._id + answerOption}><input name={question.questionName}/>{answerOption}</label>
                                    })}</div>
                            })}<input defaultValue={survey.surveyName} name='surveyName' style={{
                            display: 'none'
                        }}/>
...
ReactDOM.render(
    <GenerateFromDb/>, document.getElementById('from-database'))
